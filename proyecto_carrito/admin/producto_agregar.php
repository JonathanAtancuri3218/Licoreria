...
<?php
	...
	if($_POST['agregarProducto'] == "agregarProducto"){
		...
		$q="INSERT INTO `productos` (`id`, `nombre`, `codigo`, `categoria`, `proveedor`,`frase_promocional`, `unidad`, `precio`, `disponibilidad`, `descripcion`, `promocion`, `fecha`)
		 VALUES (NULL, '$_POST[nombre]', '$_POST[codigo]', '$_POST[categoria]','$_POST[id]', '$_POST[frase_promocional]', '$_POST[unidad]', '$_POST[precio]', '$_POST[disponibilidad]', '$_POST[descripcion]', '$_POST[promocion]', CURRENT_TIMESTAMP)";
		//echo($q);
...
?>
          ...
          <?php
             ...
             $sql3=mysqli_query($conn,"SELECT unidad from productos");
          //echo($q);
          mysqli_close($conn);
          $resource3=mysqli_num_rows($sql3);                  
       ?>
       <select name="unidad" id="unidad" class="form-control selectpicker">
	   <option value=" " >Seleccione una Unidad de medida</option>
       <?php
       if($resource3 > 0){
	    while($unidad=mysqli_fetch_array($sql3)){
        ?>
